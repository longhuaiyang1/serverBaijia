package com.baijia.lhy.controller;


import com.baijia.lhy.pojo.dto.Result;
import com.baijia.lhy.pojo.entity.UserOrder;
import com.baijia.lhy.service.IUserOrderService;
import com.fasterxml.jackson.databind.util.JSONPObject;
import net.minidev.json.JSONObject;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RestController;

import java.time.Clock;
import java.time.LocalDateTime;
import java.time.ZoneId;
import java.util.LinkedHashMap;

/**
 * <p>
 *  前端控制器
 * </p>
 *
 * @author lhy
 * @since 2020-05-13
 */
@RestController
public class UserOrderController {
    @Autowired
    IUserOrderService userOrderService;

    @PostMapping("/uploadOrder")
    public Result getReceivePoints(@RequestBody JSONObject jsonObject) {
        try{
            UserOrder userOrder = new UserOrder();
            userOrder.setUserId(1);////////////////////////////////

            LinkedHashMap receivePoint = (LinkedHashMap) jsonObject.get("receivePoint");
            userOrder.setReceivePointId((Integer) receivePoint.get("id"));

            userOrder.setReceiverName(jsonObject.get("receiverUserName").toString());
            userOrder.setReceiverPhone( jsonObject.get("receiverUserPhone").toString());
            userOrder.setPayType("cash");
            userOrder.setProducts(jsonObject.getAsString("productsArray"));
            userOrder.setEstimateTotalCost(Double.valueOf(jsonObject.getAsString("totalPrice")));
//
            userOrder.setStatus(0);//0待支付

            LocalDateTime time = LocalDateTime.now(Clock.system(ZoneId.of("Asia/Shanghai")));
            userOrder.setCreateTime(time);
            userOrder.setUpdateTime(time);

            boolean success= userOrderService.save(userOrder);

            System.out.println("1111111"+jsonObject);
            Result result = new Result();
            if(success){
                result.setCode(2001);
                result.setMsg("");
                result.setData(new JSONObject());
                return  result;
            }

        }catch (Exception e){
            e.printStackTrace();
        }

        //失败
        Result result = new Result();
        result.setCode(3006);
        result.setMsg("失败");
        result.setData(new JSONObject());
        return  result;
    }
}

